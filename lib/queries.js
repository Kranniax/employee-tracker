...
class EmployeeQueries {
  ...
  async deleteEmployee() {
    ...
    try {
      ...
      console.log(
        `Employee: ${
          employees.find((e) => e.id === employeeChoice.employeeId)
            .employee_name
        } has been deleted from the database.`
      );
    } catch (error) {
  ...
  // Show total utilized department budget
  async utilizedDepartmentBudget() {
    const [departments] = await db.query(`SELECT id, name FROM department`);
    ...
    const departmentChoice = await inquirer.prompt([
      {
        type: "list",
        name: "departmentId",
        message: "Select a department to view their total utilized budget",
        choices: departments.map((dept) => ({
          name: dept.name,
          value: dept.id,
        })),
    ...
    try {
      const sql = `SELECT SUM (role.salary) AS department_budget
                   FROM employee e
                   INNER JOIN role ON e.role_id = role.id
                   INNER JOIN department on role.department_id = department.id
                   WHERE department.id = ? `;
